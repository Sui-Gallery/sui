...
let MARKET = null
let MARKET_TYPE =
    '0x3d34f61d0facddd910e8aaaf9474494b822e319d4a291b0cb9a034d2d324d96c::marketplace::Gallery'
let MARKETPLACE_ADAPTER = null
...
async function place({ address, item, type, price }) {
    return await withKioskTransaction({ address }, async (txb, kioskTx, cap) => {
        kioskTx.place({
            item,
            itemType: type,
...
async function acceptBidOnMarket({ address, type, price, item, source }) {
    return await withKioskTransaction({ address }, async (txb, kioskTx, cap) => {
...
async function executeAcceptBid({ toolbox, address, source, type, price, item }) {
    const sourceKiosk = await client.getObject({ id: source })
    const txb = await acceptBidOnMarket({ address, source: sourceKiosk.data, type, price, item })
    await executeTransactionBlock(toolbox, txb)
        .then((r) => {
            console.log(r)
...
async function run_scenario_3() {
    ...
    const bidToolbox = await setupSuiClient()
    const sourceKiosk = await executeAddExtension({ toolbox: bidToolbox })
    console.log(sourceKiosk)
